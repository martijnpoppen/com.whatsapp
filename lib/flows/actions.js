exports.init = async function (homey) {
    ...
    const action_wa_group_send_message = homey.flow.getActionCard('action_wa_group_send_message');
    action_wa_group_send_message.registerRunListener(async (args, state) => {
        await args.device.onCapability_SendMessage(args, true);
    });

    const action_wa_group_send_message_document = homey.flow.getActionCard('action_wa_group_send_message_document');
    action_wa_group_send_message_document.registerRunListener(async (args, state) => {
        await args.device.onCapability_SendMessage(args, 'document', true);
    });

    const action_wa_group_send_message_video = homey.flow.getActionCard('action_wa_group_send_message_video');
    action_wa_group_send_message_video.registerRunListener(async (args, state) => {
        await args.device.onCapability_SendMessage(args, 'video', true);
    });

    const action_wa_group_send_message_audio = homey.flow.getActionCard('action_wa_group_send_message_audio');
    action_wa_group_send_message_audio.registerRunListener(async (args, state) => {
        await args.device.onCapability_SendMessage(args, 'audio', true);
    });

    const action_wa_group_send_message_image = homey.flow.getActionCard('action_wa_group_send_message_image');
    action_wa_group_send_message_image.registerRunListener(async (args, state) => {
        await args.device.onCapability_SendMessage(args, 'file', true);
    });

    const action_wa_group_send_message_image_token = homey.flow.getActionCard('action_wa_group_send_message_image_token');
    action_wa_group_send_message_image_token.registerRunListener(async (args, state) => {
        await args.device.onCapability_SendMessage(args, 'file', true);
    });
