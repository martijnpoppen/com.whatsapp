const init = async function (homey) {
    ...
    const action_send_message_location = homey.flow.getActionCard('action_send_message_location');
    action_send_message_location.registerRunListener(async (args, state) => {
        await args.device.onCapability_SendMessage(args, 'location');
    })
    ...
    const action_send_message_poll = homey.flow.getActionCard('action_send_message_poll');
    action_send_message_poll.registerRunListener(async (args, state) => {
        await args.device.onCapability_SendMessage(args, 'poll');
    })
