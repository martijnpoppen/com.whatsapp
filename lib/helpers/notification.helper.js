...
module.exports = class notificationHelper {
    constructor(homey, webhook, homeyId) {
        this.homey = homey;
    ...
    async handleDeviceNotification({ query, body }) {
        console.log(query.homey, this.homeyId)
        if(query && query.homey && query.homey !== this.homeyId) {
            this.homey.app.log(`[NTFY] - 0. handleDeviceNotification - Homey doesn't match!`, );
